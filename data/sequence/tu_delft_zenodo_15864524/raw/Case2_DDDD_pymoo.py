# -*- coding: utf-8 -*-
"""
Created on Sun Dec  1 13:09:30 2024
...
load_Nxx = 1. # [lb]
load_Nyy = 1 # [lb]
TEMP1=[90]*10 + [+45,-45]*2 + [90]*2 + [+45, -45]*3 + [90]*2 + [+45, -45]*4
#TEMP2=[+45,-45]+[90]*10+[+45,-45]+[90]*8+[+45,-45]+[90]*8
#TEMP3=[90]*4+[+45,-45]*2 +[90]*16+[+45,-45]+[90]*6
#TEMP4=[90]*2 + [+45,-45] + [90]*6 + [+45, -45] + [90]*8 + [+45, -45]+[90]*10
#TEMP5=[90]*8 + [+45,-45] + [90]*2 + [+45, -45] + [90]*2 + [+45, -45]+[90]*2+[45, -45]*6

#TEMP=[TEMP1,TEMP2,TEMP3,TEMP4,TEMP5]
tmp = TEMP1
ref_stack = tmp + tmp[::-1]

assert len(ref_stack) == 64
...
def objective(x): #NOTE to be minimized
    a1_deg, a2_deg, a3_deg, a4_deg = x[:4]
    a1_deg = round(a1_deg, 0)
    a2_deg = round(a2_deg, 0)
    a3_deg = round(a3_deg, 0)
    a4_deg = round(a4_deg, 0)
    prop_DD = n_double_plate(h, [a1_deg, a2_deg, a3_deg, a4_deg], laminaprop)
    prop_DD.stack = [a1_deg, a2_deg, a3_deg, a4_deg]
    lambda_cb = calc_buckling(prop_DD)
    lambda_cs = calc_failure_load_Haftka(prop_DD)
    p = 0.08
    ref_obj = (1-p)*min(ref_lambda_cs, ref_lambda_cb)
    obj = (1-p)*min(lambda_cs, lambda_cb)
    hist.append([a1_deg, a2_deg, a3_deg, a4_deg, lambda_cb, lambda_cs, obj])
    # NOTE uncomment here to make the objective driven by buckling
    #return 1/lambda_cb
    # NOTE uncomment here to make the objective driven by failure criterion
    # return 1/lambda_cs
...
class Problem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        out['F'] = objective(x) # objective function
...
print(res)
# NOTE x has 16 layers to build at the end a laminate with 64 layers
X = np.round(res.X)
# NOTE balancing
X = np.vstack((X, -X)).T.flatten()
X = X.tolist()
